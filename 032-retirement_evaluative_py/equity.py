...
class Equity:
    ...
    def __valid_ROR(self, filename):
        with open(filename, "r") as f:
            ...
            for i in range(len(lst_lines)):
                ...
                date, Index, Div = line.split(",")
                if not self.__valid_date(date):
                    sys.exit(20)
                    # raise ValueError(f"Line {lst_lines.index(line)}: '{line}' does not have a valid date")
                pattern = r'^\d+(\.\d+)?$'
                if not (bool(re.match(pattern, Index)) and bool(re.match(pattern, Div))):
                    sys.exit(20)
                    # raise TypeError(f"Line {lst_lines.index(line)}: '{line}' does not have numeric Index or Div")
                if not (float(Index) > 0 and float(Div) >= 0):
                    sys.exit(20)
                    # raise ValueError(f"Line {lst_lines.index(line)}: '{line}' does not have a positive Index or Div")
                if i==1:
                    continue
    ...
    def __set_ROR(self, ROR_filename):
        ...
        with open(ROR_filename, "r") as f:
            lst_lines = [i.strip() for i in f.readlines()]
            prev_Index = float(lst_lines[1].split(",")[1])
            for i in range(len(lst_lines)):
                if i==0:
                    continue
                line = lst_lines[i]
    ...
    def calculate_plan(self):
        ...
        for date in self.date_list:
            ...
            Index_ROR = dict_ROR.get(date).get("IndexROR")
            Div_ROR = dict_ROR.get(date).get("Div")
            total_ROR = Index_ROR + Div_ROR
            total_ROR_lst.append(total_ROR)
            balance = ((1 + total_ROR) * balance)+ monthly_contribution
            result.append(balance)
